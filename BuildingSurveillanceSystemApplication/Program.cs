...
namespace BuildingSurveillanceSystemApplication
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            securitySurveillanceHub.ConfirmExternalVisitorEntersBuilding(1, "Andrew", "Jackson", "The Company", "Contractor", DateTime.Parse("12 May 2022 11:00"), 1);
            securitySurveillanceHub.ConfirmExternalVisitorEntersBuilding(2, "Jane", "Davidson", "Another Company", "Lawyer", DateTime.Parse("12 May 2022 12:00"), 2);

            securitySurveillanceHub.ConfirmExternalVisitorExitsBuilding(1, DateTime.Parse("12 May 2022 13:00"));
            securitySurveillanceHub.ConfirmExternalVisitorExitsBuilding(2, DateTime.Parse("12 May 2022 15:00"));
    ...
    public class EmployeeNotify : Observer
    {
        ...
        public override void OnCompleted()
        {
            ...
            foreach (var externalVisitor in _externalVisitors)
            {
                ...
                Console.WriteLine($"{externalVisitor.Id,-6}{externalVisitor.FirstName,-15}{externalVisitor.LastName,-15}" +
                    $"{externalVisitor.EntryDateTime.ToString("dd MMM yyyy hh:mm:ss tt"),-25}" +
                    $"{externalVisitor.ExitDateTime.ToString("dd MMM yyyy hh:mm:ss tt"),-25}");
            }
            Console.WriteLine();
            Console.WriteLine();
        }
            ...
            if (externalVisitor.EmployeeContactId == _employee.Id)
            {
                ...
                if (externalVisitorListItem == null)
                {
                    ...
                    Console.WriteLine($"{_employee.FirstName + " " + _employee.LastName}, " +
                        ...
                        $"FirstName({externalVisitor.FirstName}), LastName({externalVisitor.LastName}), " +
                        $"entered the building, DateTime({externalVisitor.EntryDateTime.ToString("dd MMM yyyy hh:mm:ss tt")})");
    ...
    public class SecurityNotify : Observer
    {
        ...
        public override void OnNext(ExternalVisitor value)
        {
            ...
            if (externalVisitorListItem == null)
            {
                ...
                Console.WriteLine($"Security notification: Visitor Id({externalVisitor.Id}), " +
                        $"FirstName({externalVisitor.FirstName}), LastName({externalVisitor.LastName}), " +
                        $"entered the building, DateTime({externalVisitor.EntryDateTime.ToString("dd MMM yyyy hh:mm:ss tt")})");
            ...
            else
            {
                if (externalVisitorListItem.InBuilding == false)
                {
                    ...
                    Console.WriteLine($"Security notification: Visitor Id({externalVisitor.Id}), " +
                        $"FirstName({externalVisitor.FirstName}), LastName({externalVisitor.LastName}), " +
                        $"exited the building, DateTime({externalVisitor.ExitDateTime.ToString("dd MMM yyyy hh:mm:ss tt")})");
    ...
    public class SecuritySurveillanceHub : IObservable<ExternalVisitor>
    {
        ...
        {
            if (_externalVisitors.Where(e => e.InBuilding).Count() == 0)
            {
                foreach (var observer in _observers)
                {
